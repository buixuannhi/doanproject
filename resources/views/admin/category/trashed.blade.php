            ...
            @foreach($cats as $model)
                <tr>
                    ...
                    <td class="text-right">
                        ...
                        <a href="{{route('category.forcedelete', $model->id)}}" onclick="return confirm('bạn có chắc muốn xóa không?')" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                        <a href="{{route('category.restore', $model->id)}}" onclick="return confirm('yes', 'khôi phục thành công')" class="btn btn-success"><i class="fas fa-trash-restore"></i></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
